/**
 * Deep Link Template Preview: /t/[slug]
 *
 * Handles universal links (cadence.app/t/{slug}).
 * Fetches the template by slug and displays a preview.
...
export default function DeepLinkTemplateScreen() {
  ...
  const handleClone = useCallback(async () => {
    ...
    setIsCloning(true);
    try {
      await cloneTemplate(supabase, user.id, template.id);
      Alert.alert('Cloned!', 'Program added to your library as a draft.', [
        { text: 'View Library', onPress: () => router.replace('/(tabs)/program/saved-programs') },
        { text: 'OK' },
      ]);
    } catch (err) {
  ...
  if (notFound || !template) {
    return (
      <ThemedView style={styles.centered}>
        ...
        <ThemedText themeColor="textSecondary" style={{ marginTop: 8, textAlign: 'center' }}>
        ...
        >
          <ThemedText>Go to Cadence</ThemedText>
        </Pressable>
      </ThemedView>
    );
  }
...
const styles = StyleSheet.create({
  ...
  backButton: { marginTop: 24, borderRadius: Radii.medium, paddingHorizontal: 20, paddingVertical: 12 },
});
